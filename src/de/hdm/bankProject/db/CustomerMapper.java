...
public class CustomerMapper extends DataMapper<Customer> {
	...
	public void reCreateTable() {
		removeTable();
		String sqlString = "create Table customers" + " (" + " id int NOT NULL default 0, "
	...
	public void fillTable(String dataString) {
		String sqlString = "INSERT INTO customers (id,firstName,lastName) VALUES " + dataString;
		Connection con = DBConnection.connection();
		Statement stmt;
		try {
			stmt = con.createStatement();
	...
	public Customer findByKey(int id) {
		...
		try {
			...
			ResultSet rs = stmt.executeQuery(
					"SELECT id, firstName, lastName FROM customers " + "WHERE id=" + id + " ORDER BY lastName");
			/*
	...
	public Vector<Customer> findAll() {
		...
		try {
			Statement stmt = con.createStatement();
			ResultSet rs = stmt
					.executeQuery("SELECT id, firstName, lastName " + "FROM customers " + "ORDER BY lastName");
	...
	public Vector<Customer> findByLastName(String name) {
		...
		try {
			Statement stmt = con.createStatement();
			ResultSet rs = stmt.executeQuery("SELECT id, firstName, lastName " + "FROM customers "
	...
	public Customer insert(Customer c) {
		...
		try {
			...
			if (rs.next()) {
				/*
				 * c erhaelt den bisher maximalen, nun um 1 inkrementierten Primaerschluessel.
				 */
				c.setId(rs.getInt("maxid") + 1);
				stmt = con.createStatement();
				// Jetzt erst erfolgt die tatsaechliche Einfuegeoperation
				stmt.executeUpdate("INSERT INTO customers (id, firstName, lastName) " + "VALUES (" + c.getId() + ",'"
		 ...
		 * uebergeben werden, waere die Anpassung des Customer-Objekts auch ohne diese
		 * explizite Rueckgabe ausserhalb dieser Methode sichtbar. Die explizite
		 * Rueckgabe von c ist eher ein Stilmittel, um zu signalisieren, dass sich das
	...
	public Customer update(Customer c) {
		...
		try {
			Statement stmt = con.createStatement();
			stmt.executeUpdate("UPDATE customers " + "SET firstName='" + c.getFirstName() + "', " + "lastName='"
					+ c.getLastName() + "' " + "WHERE id=" + c.getId());
		} catch (SQLException e2) {
			e2.printStackTrace();
		}
	 ...
	 *
	 * @param c
	 *            der Kunde, dessen Konten wir auslesen moechten
	 * @return ein Vektor mit saemtlichen Konto-Objekten des Kunden
	 */
	public Vector<Account> getAccountsOf(Customer c) {
		/*
		 * Wir bedienen uns hier einfach des AccountMapper. Diesem geben wir einfach den
		 * in dem Customer-Objekt enthaltenen Primaerschluessel.Der CustomerMapper loest
		 * uns dann diese ID in eine Reihe von Konto-Objekten auf.
		 */
